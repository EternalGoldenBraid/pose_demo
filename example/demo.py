...
def main():
    ...
    eval_dataset = load_data(datapath=dataroot)
    ...
    obj_idx = 1
    obj_codebook = load_codebooks(model_net=model_net_ove6d, eval_dataset=eval_dataset)[obj_idx]
    ...
    cloud, cam_K = load_cloud(obj_idx=obj_idx, data_path=data_path)
    cloud *= 10 # TODO SCALE IS OFF
    cam_K.to(DEVICE)
    pose_estimator = PoseEstimator(cfg=cfg, cam_K=cam_K, 
        ...
        while True:
            ...
            if not len(masks) == 0:
                ...
                R, t = pose_estimator.estimate_pose(
                            obj_depth=torch.tensor((depth_image*mask[...,0]).astype(np.int8)).squeeze(),
                            obj_mask=masks[0])

                render_cloud(point_cloud=cloud, cam_K=cam_K, R=R, t=t, image=color_image)
